...
function renderStats() {
  ...
  const backgroundLabel = deploymentMode === "worker" ? "Worker 后台轮询运行中" : "Docker 后台轮询运行中";
...
async function sendNotification(text, product, shop) {
  const response = await fetch("./api/notify", { method: "POST", headers: { "content-type": "application/json", "x-admin-token": sessionAdminToken }, body: JSON.stringify({ text, product: product?.name || product, shop: shop?.name || shop, stock: product?.stock }) });
